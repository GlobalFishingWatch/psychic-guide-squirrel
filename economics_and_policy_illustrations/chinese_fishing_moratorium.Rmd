...
```{r}
# Not working
# draw.a.plot<-  function(i){
#   g <- ggmap::ggmap(ggmap::get_map(location = c(125,30,125,30), zoom = 4,  maptype = c("satellite"), source = 'google'))+
#   geom_point(data = removed_vessels_effort_2015 %>% 
#                filter(dh == day_hours[i]),
#              aes(x=lon, y=lat), color="red", size = 1)
#   
...
#   lapply(1:length(day_hours), function(i) {
#     draw.a.plot(i)
#   })
# }
# 
# animation::saveGIF(loop.animate(), interval = .5, movie.name="happy2015.gif")
```
