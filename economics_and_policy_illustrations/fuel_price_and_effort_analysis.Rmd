...
weekly_effort_by_mmsi %>% 
  ...
  geom_point()+
  geom_smooth()+
  ggtitle("Distance traveled per week per vessels")+
  theme_minimal()
```
...
detrended_weekly_active_hours <- nested_weekly_df %>% 
  mutate(detrended_model = map(grouped_data, detrending_function, y_var = "active_hours", time_step = "weekly"),
         residuals = purrr::map2(grouped_data, detrended_model, add_residuals, "residuals"),
         detrended_plots = trelliscopejs::map_plot(residuals, detrended_plot),
         fuel_plots = trelliscopejs::map_plot(grouped_data, fuel_plot, var = "log(active_hours)"),
         fuel_coefs = purrr::map(ungrouped_data, get_fuel_coefs, y_var = "log(active_hours)", time_step = "weekly"))
...
detrended_weekly_hours_distance <- nested_weekly_df %>% 
  mutate(detrended_model = map(grouped_data, detrending_function, y_var = "hours_distance", time_step = "weekly"),
         residuals = purrr::map2(grouped_data, detrended_model, add_residuals, "residuals"),
         detrended_plots = trelliscopejs::map_plot(residuals, detrended_plot),
         fuel_plots = trelliscopejs::map_plot(grouped_data, fuel_plot, var = "log(hours_distance)"),
         fuel_coefs = purrr::map(ungrouped_data, get_fuel_coefs, y_var = "log(hours_distance)", time_step = "weekly"))
...
detrended_monthly_active_hours <- nested_monthly_df %>% 
  mutate(detrended_model = map(grouped_data, detrending_function, y_var = "active_hours", time_step = "monthly"),
         residuals = purrr::map2(grouped_data, detrended_model, add_residuals, "residuals"),
         detrended_plots = trelliscopejs::map_plot(residuals, detrended_plot),
         fuel_plots = trelliscopejs::map_plot(grouped_data, fuel_plot, var = "log(active_hours)"),
         fuel_coefs = purrr::map(ungrouped_data, get_fuel_coefs, y_var = "log(active_hours)", time_step = "monthly"))
...
detrended_monthly_hours_distance <- nested_monthly_df %>% 
  mutate(detrended_model = map(grouped_data, detrending_function, y_var = "hours_distance", time_step = "monthly"),
         residuals = purrr::map2(grouped_data, detrended_model, add_residuals, "residuals"),
         detrended_plots = trelliscopejs::map_plot(residuals, detrended_plot),
         fuel_plots = trelliscopejs::map_plot(grouped_data, fuel_plot, var = "log(hours_distance)"),
         fuel_coefs = purrr::map(ungrouped_data, get_fuel_coefs, y_var = "log(hours_distance)", time_step = "monthly"))
